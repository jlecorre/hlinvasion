...
{
	if ( !HasConditions( bits_COND_ENEMY_OCCLUDED ) && flDist <= 4096 /*&& flDot >= 0.5 */&& NoFriendlyFire() )
	{
		TraceResult	tr;
...
void CHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (ptr->iHitgroup == 11)
	{
		...
		if ( ( GetBodygroup( 1 ) == HEAD_GRUNT || GetBodygroup( 1 ) == HEAD_HELMET ) && (bitsDamageType & (DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB)))
		{
			// absorb damage
			flDamage -= 20;
			if (flDamage <= 0)
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( InSquad() )
			{
				if ( g_iSkillLevel == SKILL_HARD && HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) && pev->weapons & HGRUNT_HANDGRENADE )
				{
					if (FOkToSpeak())
					{
						SENTENCEG_PlayRndSz( ENT(pev), "HG_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
