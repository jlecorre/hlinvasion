...
public:
	...
	void	CheckAmmo		( void );						// vérifie qu'il a des munitions
...
void CSniper :: SetYawSpeed ( void )
{
	...
	pev->yaw_speed = ys;*/
	pev->yaw_speed = 360;	// rotation en degrés par seconde, toujours identique pour le moment
}
...
void CSniper :: Spawn()
{
	...
	SetBodygroup ( HEAD_GROUP, RANDOM_LONG(0,2) );	// tête aléatoire
...
CBaseEntity *CSniper :: Kick( void )
{
	...
	vecStart.z += pev->size.z * 0.5;						// du monstre et de la moitié
															// de la hauteur de la "boite"
	Vector vecEnd = vecStart + (gpGlobals->v_forward * 70); // jusqu'à 70 unités vers l'avant
...
//-----------------------------------------
//
// CheckAttacks - éjection du fusil
//------------------------------------------.
...
//-----------------------------------------
//
// Démembrage - éjection des petits bouts sanglants
//------------------------------------------.
...
//-------------------
// tir
// redéfini car la classe de base
