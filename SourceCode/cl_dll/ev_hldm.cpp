...
/*		gHUD.m_Particules.AddParticule ( 
	...
	if ( iParticules == 1 )
	{
		...
		case CHAR_TEX_GRATE:
			break;				//seulement des étincelles
	...
	if ( chTextureType == CHAR_TEX_XENO )
	{
		...
		gHUD.m_Particules.AddDecal (
			vecOrigin + vecNormal.Normalize() * 0.1,		//décollé du mur
			angDir,
		...
		decal = 0;	// les décals habituels ne s'exécutent pas
	}
...
================
*/
void EV_HLDM_FireBullets( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float *vecSpread, float flDistance, int iBulletType, int iTracerFreq, int *tracerCount, int iTraverseMur )
{
	int i;
	pmtrace_t tr;
	int iShot;
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		...
		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, vecSpread, 2048, BULLET_PLAYER_BUCKSHOT_DOUBLE, 0, &tracerCount[idx-1] );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, VECTOR_CONE_10DEGREES, 2048, BULLET_PLAYER_BUCKSHOT_DOUBLE, 0, &tracerCount[idx-1] );
	}
...
void EV_FireMP5( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, VECTOR_CONE_7DEGREES, 8192, BULLET_PLAYER_MP5, 2, &tracerCount[idx-1] );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, VECTOR_CONE_7DEGREES, 8192, BULLET_PLAYER_MP5, 2, &tracerCount[idx-1] );
...
void EV_FireFSniper( event_args_t *args )
{
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, Vector(0,0,0)/*VECTOR_CONE_1DEGREES*/, 8192, BULLET_PLAYER_SNIPER, 1, &tracerCount[idx-1] );
