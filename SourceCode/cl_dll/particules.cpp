...
void	CHudParticules :: AddDecal ( vec3_t origin, vec3_t angles, char model [64], float deathtime, int flags )
{
	...
	if ( pmtrace.fraction == 0.0 && pe && pe->solid == SOLID_BSP && entity != 0 )
	{
		...
		// contre un mur et contre une entité
		...
		// pas de décal sur les surfaces invisibles
	...
	// A FAIRE : enlever les décals du tank
...
int CHudParticules :: Draw	( float flTime )
{
	...
	for ( int iIndex = 0; iIndex < MAX_PARTICULES ; iIndex ++ )
	{
		...
		if ( m_pParticules[iIndex].flags & FLAG_PARTICULE_OUTRO2  )
		{
			...
			{
				m_pParticules[iIndex].velocity = m_pParticules[iIndex].velocity.Normalize() * max( 0, m_pParticules[iIndex].velocity.Length() - 10 * delta );
			}
		...
		if ( m_pParticules[iIndex].flags & ( FLAG_PARTICULE_OUTRO1 | FLAG_PARTICULE_OUTRO2 | FLAG_PARTICULE_OUTRO3 ) )
		{
			if ( m_pParticules [ iIndex ].deathtime - flTime < 2.5 )
			{
				...
				m_pParticules [ iIndex ].angles.y = max ( 0, m_pParticules [ iIndex ].angles.y );
			}
		}
	...
	for ( int iDecal = 0; iDecal < MAX_DECALS ; iDecal ++ )
	{
		...
		if ( m_pDecals[iDecal].flags & FLAG_DECAL_SG )
		{
			if ( m_pDecals[iDecal].iRndSensX < 8 )
			{
				...
				m_pDecals[iDecal].iRndSensY = m_pDecals[iDecal].iRndSensX = min ( 8, m_pDecals[iDecal].iRndSensX );
			}
			continue;
		}
		...
		if ( m_pDecals[iDecal].flags & FLAG_DECAL_XENO )
		{
			if ( m_pDecals[iDecal].iRndSensX < 3 )
			{
				...
				m_pDecals[iDecal].iRndSensY = m_pDecals[iDecal].iRndSensX = min ( 3, m_pDecals[iDecal].iRndSensX );
			}
			continue;
		}
		...
		if ( m_pDecals[iDecal].entity != 0  )
		{
			...
			//suit le déplacement d'une entité
...
//=====================================
//
// tracé des particules et des decals
...
//controle du nombre de quads dessinés
...
//
// activation du système de decals
// et de particules
// depuis le serveur
//
